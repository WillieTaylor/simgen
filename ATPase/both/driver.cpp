...
FILE	*save;
	...
	fclose(save);
	C->idata[1] = 1; // set flag for C-ring loading
...
int	before, after, loading = C->idata[1];
...
// kick free protons out of the membrane
	...
	Pi(held) Pi(cytH) Pi(perH) Pi(allH) Pi(badH) Pi(loading) NL
	if (badH > allH*0.1 && frame < 500) return;
// compile list of ionisable residues (just E58 in Cring)
	nprot = 0;
	FOR(i,C->kids) FOR(j,C->child[i]->kids) FOR(k,C->child[i]->child[j]->kids)
	{ Cell *res = C->child[i]->child[j]->child[k]; char aa = 'A'+res->idata[0];
		if (aa != 'E') continue;
		prot[nprot++] = res;
	...
	FOR(i,A->kids) FOR(j,A->child[i]->kids) FOR(k,A->child[i]->child[j]->kids)
	{ Cell *res = A->child[i]->child[j]->child[k]; char aa = 'A'+res->idata[0];
		if (aa!='E' && aa!='D' && aa!='H') continue;
		prot[nprot++] = res;
	}
	FOR(i,nprot) prot[i]->idata[1] = i;
	Pi(nprot) Pi(ring) NL
	n = nprot+2;
		...
		FOR(j,nbags) { int linked = 0;		// loops over all protons
			...
			{ Cell	*H = memb->child[j]->child[k];
			  Cell	*rin = H->link[0].to;	// H has incoming link from a residue
				...
				{ int	r = rin->idata[1];	// position of rin in prot[]
					if (rin->resn==58 && rin->idata[0]==4) { // link is from C-ring E58
...
// print the transitions
	FOR(i,n) { int r; char a;
		...
		if (i == nprot) NL
		if (i<nprot) {
			r = prot[i]->resn; a = 'A'+prot[i]->idata[0];
		}
		if (i==nprot) { r = -1; a = 'p'; }
		if (i >nprot) { r = -1; a = 'c'; }
		printf("%3d%c  ", r,a);
