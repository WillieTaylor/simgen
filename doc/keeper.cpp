...
float	radius = model->sizes[cell->level]*0.5,
	...
	if (push < -NOISE) { // confine to shell (+/-margin)
		push = -push; shell = 1;
	} else { shell = 0; }
	if (push < NOISE) return 0;
	if ((int)cell->endN.z == 1234) return 1; // ends not set yet
	if ((int)cell->endC.z == 1234) return 1; // ends not set yet
	shift = cell->xyz - child->xyz;		// shift from child to zero
	if (shift.len() < radius) return 0;	// within the sphere
	shift *= push;				// pull-in wayward
	moveCell(child,shift,1);
	return 1;
}
...
//	radius -= model->sizes[child->level]*0.5; // keep totally inside
	pale = radius;
	shell = 1; // tube always confined to shell (+/-margin)
	if (cell->sort==0) shell = 0;	// except in loops
	if (push < -NOISE) {
		push = -push; closed = 1;
	} else { closed = 0; }
	if (push < NOISE) return 0;
	if (cell->sort==0) closed = 0;	// loops always have open ends
	if ((int)cell->endN.z == 1234) return 1; // ends not set yet
	if ((int)cell->endC.z == 1234) return 1; // ends not set yet
	if (vdif(cell->endN,cell->endC) < NOISE) return 1;
	if (closed) {
		dlast = vdif(cell->endN, child->xyz);
		dnext = vdif(cell->endC, child->xyz);
	...
	if (closed) { // confine within segment
		if (in) { // over line segment
			...
			shift = (over - child->xyz).norm();
		} else { // over an end-cap
			if (dlast < dnext) {
		...
		if (d < pale) { // inside the tube
			if (!shell) return 0;
			if (d < pale*margin) { // needs shifting outwards
				dif = pale-d;
				push *= dif;
				shift = child->xyz - over;
			} else { return 0; }
		} else { // needs shifting inwards
...
float	radius = model->sizes[cell->level]*0.5,
	...
	if (push < -NOISE) { // confine to shell (+/-margin)
		...
		shell = 0;
		if (vdif(cell->xyz,child->xyz) < NOISE) return 0;
	}
	if (push < NOISE) return 0;
