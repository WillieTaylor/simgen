...
void fixBPbonds ( Cell***, int, int );
void fixBPbumps ( Cell***, int, int );
...
int	fix = 5;
	...
	{ Cell  ***stem = new Cell**[kids];
		DO(i,kids) stem[i] = new Cell*[2];
		n = 0;
		DO(i,kids) { Cell *pi = cell->child[i];
		...
		fixBPbumps(stem,n,fix);
		fixBPbonds(stem,n,fix);
		m = n-1;
		...
		axis = (pole.B-pole.A).norm();		// unit vector along the axis
		// move the Ps to the tube surface
		twist = 1.0; //1.25;		// angle between basepaired Ps
		DO(i,n) { Cell *ci = stem[i][0], *wi = stem[i][1];
			...
			mid = c & w;
			spoke = mid + ((ci->xyz - c)+(wi->xyz -w)).getVec(radius); // bisector of WC angle
			if (drand48() < 0.5) {
				c = spoke.get_rot(pole, twist) + axis*BPrise; 
				ci->xyz = ci->xyz & c;
			} else {
				w = spoke.get_rot(pole,-twist) - axis*BPrise;
				wi->xyz = wi->xyz & w;
			}
		}
		fixBPbumps(stem,n,fix);
		...
		fixBPbonds(stem,n,fix);
/*
DO(i,n) { Cell *pi = cell->child[i], *ci = pi->child[0], *wi = pi->child[1];
printf("ATOM%7d  CA  GLY A%4d     %7.3f %7.3f %7.3f  0.00  0.00\n", i,i,ci->xyz.x,ci->xyz.y,ci->xyz.z);
}

DO(i,n) { Cell *pi = cell->child[i], *ci = pi->child[0], *wi = pi->child[1];
printf("ATOM%7d  CA  GLY A%4d     %7.3f %7.3f %7.3f  0.00  0.00\n", i,i,wi->xyz.x,wi->xyz.y,wi->xyz.z);
}
...
{ int	i = 1;
	...
	printf("ATOM%7d  CA  GLY C%4d     %7.3f %7.3f %7.3f  0.00  0.00\n", i,i,c.x,c.y,c.z);
	i = 2;
	printf("ATOM%7d  CA  GLY C%4d     %7.3f %7.3f %7.3f  0.00  0.00\n", i,i,mid.x,mid.y,mid.z);
	printf("TER ATOM\n");
}
exit(1);
*/
	}
	DO(i,kids) fixRNAstem(cell->child[i], weight);
}

void fixBPbonds ( Cell ***stem, int n, int fixes ) {
// patch-up local BP and backbone geometry
float	bondBP  = 2.5;
float	bondPP  = 0.9;
float	bondPxP = 1.7;
float	kick = 0.3;
	DO(j,fixes) { // fixing cycles
		DO(i,n) // fix BP separation
...
void fixBPbumps ( Cell ***stem, int n, int fixes ) {
// patch-up local BP and backbone geometry
float	d, bump = 2.0;
float	kick = 0.3;
	DO(k,fixes) { // fixing cycles
		DO(i,n) // fix BP separation
		{ Cell *ci = stem[i][0], *wi = stem[i][1];
			DO(j,n) // fix bumps with Watson i
