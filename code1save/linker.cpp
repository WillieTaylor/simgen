		...
		if (moltype==1) { // Nucleic acid chain
			...
			if (level==depth-2 && cell->type==2) {	// bond domain tube ends with 0 length bond
				...
				{ Cell	 *wC, *cC, *cN, *wN=0;
					...
					DO(i,cell->kids) { Cell *kidi = cell->child[i];
...
Cell	*ats = at->parent->parent, // stem
	*to, *tob, *tos;
	...
	LOOP { // follow loop to its end (loop has sort=0, basepair and stem levels have sort=1)
...
//	if (level && cell->parent->solid > 0) return; 
	if (cell->link) { float d;
		...
		setLinks(bondlen);
		DO(i,nlinks) { Cell *link = cell->link[i].to; int type, brittle; float snap;
			if (link==0) continue;
