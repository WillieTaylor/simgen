...
int sortInts ( const void *ac, const void *bc )
{
        int   *a = (int*)ac, *b = (int*)bc;
        if (*a < *b) return -1;
        if (*a > *b) return  1;
        return 0;
}
...
int	close[MAXHOLD];
	...
	if (kids < SWITCH || flat) {	// use pairwise
		for (i=0; i<kids-1; i++) {
			a = cell->child[i];
			if (a->busy || a->empty) continue;
			for (j=i+1; j<kids; j++) {
				b = cell->child[j];
				if (b->busy || b->empty) continue;
	...
	{ int   got, span = 5+(int)sqrt((float)kids);
		for (i=0; i<kids; i++) {
			...
			qsort(close,m,sizeof(int),sortInts);
			// WAS sort(close,m);
