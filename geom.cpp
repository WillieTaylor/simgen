...
Vec Mat::get_Euler () {
// return the Euler angles of the rotation matrix R
// signs set -ve for following call of cell->spinS(Vec)
float   x = atan2( C.y, C.z ),
        y = atan2(-C.x, sqrt(C.y*C.y + C.z*C.z)),
        z = atan2( B.x, A.x);
        return Vec(-x,-y,-z);
}
...
float	d = Mat(a,b,c).det();
	...
	x = y^z;
	return Mat( x.getNorm(), y.getNorm(), z.getNorm() );
}
...
{	// TRUE if segment intersects the (components of) triangle a,b,c (in e)
	if (e.x < 0.0) return 0;
	if (e.y < 0.0) return 0;
	if (e.z < 0.0) return 0;
	if (e.z > 1.0) return 0;
...
bool cut_tri ( Vec a, Vec b, Vec c, Vec p, Vec q ) 
{	// TRUE if line segment p-q intersects the triangle a,b,c (was line2tri())
	Seg s = Seg(p,q);
	Vec e = s.hit_trif(a,b,c);
	if (e.x < 0.0) return 0;
	if (e.y < 0.0) return 0;
	if (e.z < 0.0) return 0;
	if (e.z > 1.0) return 0;
...
bool line2line ( Vec a1, Vec a2, Vec b1, Vec b2, float s ) { // as in old geom.c
// TRUE if line segments a and b overlap and are closer than s 
	Seg a = Seg(a1,a2); Seg b = Seg(b1,b2);
	return line2line(a,b,s);
}
...
Mat	M,R = Mat();
	...
	y = x^z;
	M = Mat(x.getNorm(),y.getNorm(),z.getNorm());
	d = d*M;		// a-b to X
...
// return the Euler angles of the rotation matrix R
// signs set -ve for following call of cell->spinS(Vec)
float   x = atan2( R.C.y, R.C.z ),
        y = atan2(-R.C.x, sqrt(R.C.y*R.C.y + R.C.z*R.C.z)),
        z = atan2( R.B.x, R.A.x);
	return Vec(-x,-y,-z);
}
