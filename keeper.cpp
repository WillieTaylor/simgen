...
Vec	mid, shift;
	...
	if (cell->sort == 0) {     // realign loop tube with ends of the chain
		shift = (cell->junior->xyz - cell->senior->xyz).getNorm();
...
int	moltype = model->moltype,
	...
	if (d < pale) {
		push = (pale-d)*strict;
		shift = (child->xyz - cell->xyz).getNorm()*push;
		child->move(shift);
	}
	pale = radius*1.1;
	...
	if (d < pale) {
		push = (pale-d)*strict;
		shift = (child->endN - cell->xyz).getNorm()*push;
		child->move(shift);
	}
	d = cell->endC|child->xyz;
	if (d < pale) {
		push = (pale-d)*strict;
		shift = (child->endC - cell->xyz).getNorm()*push;
		child->move(shift);
	}
}
...
int	apply = 1;
	...
	if (a*a+b*b < 0.04) { // sphere-ish (both minor axes over 0.8-ish)
		...
		c = cell|cent;
		if (apply) move = (cent-cell).getNorm();	// move towards centre
		return c-g;
	}
