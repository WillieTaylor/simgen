...
int	place, baseN, baseC, basepair = 0, print = 0;
	...
	{ int	in = read_line(file,full);
		...
		if (strstr(line,"ALPHA")) { // read (to EOF or END) and set Hbond pairs
			Pt(# Add alpha helix H-bonds) NL
			if (in > 6) {
				setAlpha(file,toupper(line[6])); // numbering mode specified
			} else {
				setAlpha(file,'G'); // default is teratom (group) numbering
			}		
			last = 'S';
			continue;
		}
...
int	len = strlen(line), in = 0;
char	*start;
	for (int i=1; i<len; i++) {
		if (line[i-1]==' ' && line[i]!=' ') in++; 
		if (in == 2) start = line+i;
	}
	if (in == 2) return;
	if (in!=5 && in!=8) {
		Ps(line) NL
		Pt(Bad GROUP command) Pi(in) Ps(start) NL
		exit(1);
	}
	x2=y2=z2 = 0.0;
	if (in==5) sscanf(start,"%f %f %f", &x1,&y1,&z1);
	if (in==8) sscanf(start,"%f %f %f  %f %f %f", &x1,&y1,&z1, &x2,&y2,&z2);
...
Data	*param = Data::model+m;
	...
	//
	if (level < Data::depth)	// set end-points
	{ float	size = sizes[level],	// for tube and ellipsoids, size = section diameter
		fn = (float)cell->kids,
		d, axis = 1.0;
	  Vec	cent, x;
		cell->setWcent();	// sum (weighted) children into upper levels (just bp.s in nuc)
		x = cell->xyz - (cell->endN & cell->endC); 
		cell->endN += x; cell->endC += x;
		if (cell->ends) {	// ends have been read in (-ve = flag to keep given length)
		...
		{ float	sf = fn-1.0;
			...
			if (moltype==1) { // nucleic (always set ideal length)
				if (level==depth-2) // double strand tube
				{ int	to,in = -1;
					sf = 0.0;
					cent.zero();
					FOR(i,n) { Cell *b = cell->child[i];
						if (b->sort == 0) continue; // use only basepairs (sort=stem.id)
						cent += b->xyz; sf += 1.0;
						if (in<0) in = i;
						to = i;
					}
					cell->xyz = cent/sf;
					cell->endN = cell->child[in]->xyz & cell->child[in+1]->xyz;
					cell->endC = cell->child[to]->xyz & cell->child[to-1]->xyz;
					x = cell->endC - cell->endN;		// set axis direction
					// RNA = 2.3 rise/bp --> 0.115 (ie: 1/2 for duplex, 1/10 for scale)
					axis = 0.23*(sf-1.0);
					if (axis < 1.0) axis = 1.0;
				}
				if (level==depth-1) { // DNA/RNA (rungs of basepaired ladder)
					axis *= 0.7; // keep bp ends inside Ps
		...
		x.setVec(axis*0.5);	// x set to half length
		cell->endN = cell->xyz - x;
		cell->endC = cell->xyz + x;
/*
//////////////////////////////may not work with fixRNAaxis
...
int	nlinks;
	...
	while (1) { int i, io, a,b,c; Cell *from, *to;
		...
		for (i=2; from->link && i<nlinks; i++) { // 0,1 used for +/-2 in strand
			...
			if (from->link[i].to == 0) { float d;
				from->link[i].to = to;
				from->link[i].type = 15;
				from->link[i].next = -c; // -ve = not brittle (used as wt in fixSheet)
				d = from->xyz|to->xyz;
				if (mode=='P') {
					Pt(# set Hbond) Pi(i) Pi(from->resn) Pi(to->resn) Pr(d) NL
				} else {
					Pt(# set Hbond) Pi(i) Pi(from->uid) Pi(to->uid) Pr(d) NL
				}
				break;
			}
		}
...
void setAlpha ( FILE *beta, char mode )
{ // set specified links to usually to join two helices (copy of setSheet() with no sec level link) 
char	line[222];
int	nlinks;
	Pt(#) Pc(mode) NL
	while (1) { int i, io, a,b,c,type; Cell *from, *to;
		io = read_line(beta,line);
		if (io < 0 ) break;
		if (io < 10) continue;
		if (line[0] == '#') continue;
		// Hbond 123 456  5  10
		// c = strength, d = length type: 10 = i--i+3, 11 = i--i+4
		sscanf(line+6,"%d %d %d %d", &a, &b, &c, &type);
		if (type==0) type = abs(a-b);	// set type by resitypeue spacing
		if (type==3) type = 10;	// or
		if (type==4) type = 11;	// specifietype offset
		if (c>5) { Pt(#) Ps(line) NL } // echo strong ones
		if (mode=='G') {	// set links using group ids
			from = atom1cell[a]; to = atom1cell[b];
		}
		...
		for (i=0; from->link && i<nlinks; i++) {
			...
			if (from->link[i].to == 0) { float d;
				from->link[i].to = to;
				from->link[i].type = type;
				from->link[i].next = -c; // -ve = not brittle (used as wt in fixSheet)
				d = from->xyz|to->xyz;
				if (mode=='P') {
					Pt(# set Hbond) Pi(i) Pi(from->resn) Pi(to->resn) Pi(type) Pr(d) NL
				} else {
					Pt(# set Hbond) Pi(i) Pi(from->uid) Pi(to->uid) Pi(type) Pr(d) NL
				}
				break;
			}
		}
...
int	nlinks, m = ci->model;
	...
	if (nlinks>0) { int made = 0;
		...
		if (made) {
			printf(" # Cell %d linked to cell %d  (%d %d)  PDBids = %d to %d   dist = %f\n",
				ci->uid, cj->uid, type, link, ci->resn, cj->resn, vdif(ci->xyz,cj->xyz));
		} else {
