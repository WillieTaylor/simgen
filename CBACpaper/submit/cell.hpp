...
public:
	...
	int     id, kids, uid, resn;	// id = number in family kids, uid = unique id
	...
	int	atom, btom, ctom;	// low-level, sub-level, all atom number
...
void putpdb ( float );
void putpdb ( Cell* );
void putpdb ( Cell*, float );
void putpdb ( char*, Cell*, float );
void putpdb ( FILE*, Cell*, float );
