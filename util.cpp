...
int acid2num ( char *acid ) {
// return the numeric value of a three-letter amino acid code (in alphabetic order) 
char    aa3[80], aaa[4];
	strncpy(aaa,acid,3);
        strcpy(aa3,"ALAASXCYSASPGLUPHEGLYHISILEACELYSLEUMETASNPCAPROGLNARGSERTHRUNKVALTRPXXXTYRGLX");
	for (int i=0; i<80; i+=3) if (strncmp(aa3+i,acid,3)==0) return i/3;
	return -1;
}

// functions

float bell ( float x ) { // Gaussian with sigma=1, mean=0, height=1
	return exp(-x*x);
}
float bell ( float x, float s ) { // Gaussian given sigma, (mean=0, height=1)
	return exp(-x*x/(s*s));
}
float bell ( float x, float s, float m ) { // Gaussian given sigma, mean, (height=1)
	x-=m; return exp(-x*x/(s*s));
}
float bell ( float x, float s, float m, float h ) { // Gaussian given sigma, mean, height
	x-=m; return h*exp(-x*x/(s*s));
}

float sigm ( float x ) { // Sigmoidal with sigma=1, mean=0, height=1
	return 1.0/(1.0+exp(-x));
}
float sigm ( float x, float s ) { // Sigmoidal given sigma, (mean=0, height=1)
	return 1.0/(1.0+exp(-x*s));
}
float sigm ( float x, float s, float m ) { // Sigmoidal given sigma, mean, (height=1)
	x-=m; return 1.0/(1.0+exp(-x*s));
}
float sigm ( float x, float s, float m, float h ) { // Sigmoidal given sigma, mean, height
	x-=m; return h/(1.0+exp(-x*s));
}
...
void timeout () {
	timeout(0.1f); // hard coded to allow use outside simgen
	//timeout(Data::delay);
}

void timeout ( float delay ) {
long int nanos = 1000000000;
int	secs = (int)delay;
float	left = delay - (float)secs;
	nanos = (long int)((float)nanos*left);
	timeout(secs,nanos);
}

void timeout ( int nanosecs ) {
	timeout((long int)nanosecs);
}
...
int	pair = 0, floating = 0, integer = 0;
	...
	m = n;
	while (1) { 						/* DO OF DECREASING SWAP SPAN	*/
		m = m/2;
		if(m == 0) return;
		for ( i = 0; i < n-m; i++ ) {			/* DO OVER SWAPS */
			j = i;					/* DO TO BUBBLE UP */
			while (1) { int	jp;
				if (reverse) {
