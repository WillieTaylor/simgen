...
void spinsCell (Cell* );
void patchCell ( Cell*, float, int, int, int );
...
Cell	*world = Cell::world;
	FOR(i,world->kids) // don't shake the world, just its contents
	{ Cell	*child = world->child[i];
		if (child->live < 1) continue;
		if (child->parent->solid > 0) continue;
		if (child->busy || child->empty) continue;
		spinsCell(child);
...
int	moltype = param->moltype;
	...
	if (Data::frozen && cell->heat < randf()) {
		if (disp.len() < NOISE) return; // any move must be applied to levels below
		return;
	}
	level = cell->level;
...
int	moltype = param->moltype;
	n = cell->kids;
	if (cell->busy || cell->empty) return;
	if (Data::frozen && cell->heat < randf()) return; // can skip OK as spin() applied to all
	...
	FOR(i,n) spinsCell(cell->child[i]);
}

void tinker ( float weight, int low, int top, int loops ) {
Cell	*world = Cell::world;
	FOR(i,world->kids) // don't patch the world, just its offspring
	{ Cell	*child = world->child[i];
		if (child->live < 1) continue;
		if (child->parent->solid > 0) continue;
		if (child->busy || child->empty) continue;
		patchCell(child,weight,low,top,loops);
	}
}

void patchCell ( Cell *cell, float weight, int low, int top, int loops )
{
Data	*param = Data::model+cell->model;
int	level = cell->level;
...
float	dev, gev, w;
	...
	for (i=0; i<fixes && dists && bonds && n>3; i++) {
		if (cell->level+1 < top || cell->level+1 > low ) break; // refine only inside range
		if (loops && cell->level==Data::depth-1 && cell->sort==0) break; // skip loops
		in++;
		dev = gev = 0.0;
		// fix local distances
		m = (int)(5.0*randf());
	...
	for (i=0; i<n; i++) patchCell(cell->child[i], weight,low,top,loops);
}
...
int	i, end=0;
	...
	y = xyz[2] - mid;
	z = x^y;
	mat = Mat(x.getNorm(), y.getNorm(), z.getNorm()); // new unit basis vectors in mat
	wat = mat.get_inv();
