...
public:
	...
	void group ( float );			// dito with specified coupling: 1=kids, 0=cell move
	void shifter( const float );		// couples parent's centre to centroid of kids
	void spin ( const Vec, const Vec, const float ); // spin cell tree 
	void spin ( const Vec, const float );	// spin about centre--axis
	void spin ( const Seg, const float );	// spin about a line
	void spin ( const float );		// spin about a random line
	void spinX ( const float );		// spin about centre--X
	void spinY ( const float );		// spin about centre--Y
	void spinZ ( const float );		// spin about centre--Z
	void spinS ( const float, const float, const float );	// spin about centre--XYZ
	void spinS ( const Vec );		// spin about self centre--XYZ
	void spinS ( const Vec, const Vec );	// spin about given centre--XYZ
...
int sortPairs ( const void*, const void* );
void cellSort ( cellPairs*, int );
...
void slow_spinCell ( Cell*, Vec, Vec, float );	// same as spinCell but with delayed steps
...
void sortall ();				// default: scaleout
				// just atomic level
void putpdb ( FILE*, Cell*, float, char );	// into open file with specified mode
void putpdb ( FILE*, Cell*, float );	// into open file (default: temp = sec.str.type)
...
void pdbout ( FILE* );			// open filename (default centre = leave)
void pdbout ( FILE*, Cell* );		// open filename (default centre = leave)
void pdbout ( FILE*, Cell*, float );	// open filename (default centre = leave)
void pdbout ( FILE*, Cell*, float, int );	// into open file (specify centring)
