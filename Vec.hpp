...
public:
	...
	inline Vec getVec ( const Vec &v ) const {		// set to normalised given vector (unchanged)
		float s = v.len();
		return Vec( x/s, y/s, z/s );
	}
	// norm() = setNorm() = setVec(), just to be close to old norm(&x)
	inline void norm () {				// set the normalised current vector
		float s = sqrtf(x*x + y*y + z*z);
		x/=s; y/=s; z/=s;
	}
	// [sg]etNorm() pair same as basic [sg]etVec (no arg.)
	inline void setNorm () {			// set the normalised current vector
		float s = sqrtf(x*x + y*y + z*z);
		x/=s; y/=s; z/=s;
	}
	inline Vec getNorm ()  {			// return the normalised current vector
		float s = sqrtf(x*x + y*y + z*z);
		return Vec( x/s, y/s, z/s );
	}
	...
	// c = a.getVec(b); sets c to a vector length b along a (with a unchanged)
	//
	// functions
