...
char	line[222];
	...
	Data::unweight = 0;	// centroids weighted by number of kids
	DO { int n, io = read_line(run,line);
		DO {
		...
		if (line[0]=='N' && line[2]=='R') {	// NORUN flag
			Data::nolive = 1;	// skip driver, looker, helper
			continue;
		}
		if (line[0]=='N' && line[2]=='M') {	// NOMOVE flag
			Data::nomove = 1;	// skip linker, shaker, bumper, fixers, minder 
			continue;
		}
		...
		if (line[0]=='F' && line[1]=='R') {	// FROZEN [uid] (freeze remote objects)
			Data::frozen = 1;		// sets uid of a focus object (1st child)
			if (io > 7) {			// that defines a zone (d) of activity
				sscanf(line+6,"%d", &n);
				Data::frozen = n;	// -ve = follow in viewer()
			}
			continue;
		}
		if (line[0]=='H' && line[1]=='I') {	// HIDDEN [range] (hide object contents)
			Data::hidden = 1;
			if (io > 7) {
				sscanf(line+6,"%d", &n);
				Data::hidden = n;
		...
		if (line[0]=='U') {	// UNWEIGHT flag (children add unit weight to centroid)
			Data::unweight = 1;
			continue;
		}
		if (line[0]=='V') {	// VIEWAT Z distance of the eye from the focus
			Ps(line) NL
			sscanf(line+7,"%f", &s);
			Data::viewat = s;
			continue;
	...
	while (1) { char atomtype; // read in param file(s)
		...
		if (line[0] == 'M' || line[0] == 'E') { // MODEL or END to finish
			...
			FOR(i,nmodels) // set the atomic bond lengths
...
Data	*model = Data::model+n;
	...
	if (strlen(line) > 4) {
        	...
        	if (toupper(line[5])=='H') model->colours = 3; // flash Hbond in a different colour
        	if (toupper(line[5])=='F') model->colours = 4; // colour by distance from focus b/w
	}
	name = line[io-1];
	i = 0;
	while (1)
		...
		for (j=0; j<N; j++) { int in;
			...
			switch (i) {
				...
				case 2 :
					...
					//	-ve = use shell surface not simple radius
					break;
				case 3 :
					model->links[j] = in;
					//	-ve
