/*
cc -m32 -O -c lines.c
cc -m32 -O getsec.c -o getsec lines.o util/dragproj.o util/aa/stutest.o util/aa/bestrot.o util/wt/util.o util/wt/geom.o util/wt/sort.o util/aa/pdbprot.o util/aa/matrix.o util/aa/siva.o util/aa/ql.o -lm

getsec prot 1.3 1.0 0.25
...
*/
		...
		fprintf(pdbout,"\n");
		lineup(argv[1],prot,j,secstr,gapen,betaw,betae);
		bsum = 0.0;
		for (i=0; i<nres; i++) bsum += prot->Chains[j].Atoms[i].Bfact;
		fprintf(pdbout,"REMARK    Length =%4d\n", nres);
		for (i=0; i<nres; i++)
		{ Atom_ *Atoms = prot->Chains[j].Atoms;
		  float s,t; char c = toupper(secstr[i+1].seg);
			s = 1.0;
			if (c=='A') s = 3.0;
			if (c=='3') s = 2.5;
			if (c=='B') s = 2.0;
...
lineup (code,prot,chain,secstr,gapen,betaw,betae)
char	*code;
Pdbentry_ *prot;
int	chain;
Secs	*secstr;
