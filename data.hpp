...
public:
	...
	float	bumps[N];	//  size used by bumper()
				//	-ve = use bumps[] for detection but sizes[] for bump
	int	links[N];	//  number of link[] slots allocated
				//	-ve
	int	chain[N];	//  topology: number of bonds (+ve = linear)
				//	-ve = circular
	float	kicks[N];	//  kick strength of a move (used as kicks[]*0.01)
				//	-ve = no spin
	...
	static	Vec	focus;		// focus point of interest set by FROZEN (-ve, view follows)
	static	float	Eratio[E];	// range of ellipsoid shapes
	static	float	shrink, viewat;
	static	float	bondCA, bondPP, distH;
	static	float	scalein, scaleout;
	static	float	delay;		// in seconds for timeout()
	static	float	fixRNA;
	static	int	unweight;
	static	int	depth, nmodels;
	static	int	nolive, nomove, noview, putpdb;
	static	int	frame, hidden, frozen, tinker;
};
